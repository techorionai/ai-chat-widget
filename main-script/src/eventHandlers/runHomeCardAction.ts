...
const runHomeCardAction = (data: { actionIdx: number[] }) => {
  try {
    ...
    if (
      !window.$aiChatWidget.initialConfig?.homeScreenConfig?.additionalCards ||
      !Array.isArray(data.actionIdx) ||
      data.actionIdx.length === 0
    ) {
      throw new Error(
        "Invalid action index or no additional cards configured."
      );
    }

    let action: Function | string | undefined = undefined;
    const card =
      window.$aiChatWidget.initialConfig.homeScreenConfig.additionalCards[
        data.actionIdx[0]
      ];
