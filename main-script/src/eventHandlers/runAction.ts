...
const runActionEventHandler = (data: { name: string }) => {
  try {
    ...
    const enabledActionsMap: Record<string, Function | string | undefined> = {};
    ...
    if (!action) {
      throw new Error(
        `Action "${data.name}" is not enabled or does not exist.`
      );
    }
    ...
    alert(
      "An error occurred while trying to run the action: " +
