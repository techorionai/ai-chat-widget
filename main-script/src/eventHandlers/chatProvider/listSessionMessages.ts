import { ChatProviderListSessionMessagesOptions } from "../../types.js";
import logger from "../../utils/logger.js";
import sendEventToIframe from "../../utils/sendEvent.js";

const chatProviderListSessionMessagesEventHandler = async (
  data: ChatProviderListSessionMessagesOptions
) => {
  try {
    ...
    const result = await window.$aiChatWidget.chatProvider?.listSessionMessages(
      data
    );
    ...
    logger.error(
      "Error handling chatProviderListSessionMessages event:",
      error
    );
