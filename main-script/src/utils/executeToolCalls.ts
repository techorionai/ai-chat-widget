...
export const executeToolCall = async (toolCall: ToolCall): Promise<string> => {
  try {
    logger.log(`Executing tool call: ${toolCall.function.name}`);
    if (!window.$aiChatWidget) throw new Error("Chat widget not initialized");
    const fn =
      window.$aiChatWidget.initialConfig?.functionsMap?.[toolCall.function.name]
        .fn;
    if (!fn) throw new Error(`Function ${toolCall.function.name} not found`);
    ...
    return `Error executing tool call ${toolCall.function.name}: ${
...
const safeArgsParse = (args: string) => {
  try {
